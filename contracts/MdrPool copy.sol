...
import "./Lib8/Pausable.sol";

contract OdcPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 odcAtLastUserAction; // keep track of odc deposited at the last user action.
    ...
    IERC20 public immutable token; // odc token.
    ...
    /**
     * @notice Constructor
     * @param _token: Odc token contract
    ...
    /**
     * @notice Checks if the msg.sender is either the odc owner address or the operator address.
     */
    modifier onlyOperatorOrOdcOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or odc owner");
        _;
    }
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.odcAtLastUserAction;
    ...
    /**
     * @notice Unlock user odc funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrOdcOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Odc Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in ODC)
    ...
    function depositOperation(
        ...
        user.odcAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Odc Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (user.shares > 0) {
            user.odcAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.odcAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Odc Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingOdcRewards();
     ...
     * @return Returns total pending odc rewards
     */
    function calculateTotalPendingOdcRewards() public view returns (uint256) {
        // TODO-SD
        // uint256 amount = masterchefV2.pendingOdc(odcPoolPID, address(this));
        // return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingOdcRewards()) * (1e18)) / totalShares);
    }
